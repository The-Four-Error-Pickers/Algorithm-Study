...
class Solution {
    public int solution(int[][] scores) {
        ...
        **/
        ...
        int[] memo1 = new int[scores.length+1];  //근무 태도 점수의 최대값
        memo[0] = scores[0][1];
        memo1[0] = scores[0][0];
        PriorityQueue<int[]> pq = new PriorityQueue<>((o1,o2) -> o2[0] - o1[0]);
        pq.offer(new int[]{memo[0]+memo1[0], memo[0], memo[1]});
        for(int i=1; i<scores.length; i++) {
            int[] cur = scores[i];
            memo[i] = Math.max(memo[i-1], cur[1]);
            memo1[i] = Math.max(memo1[i-1], cur[0]);
            if(memo[i-1] > cur[0] && memo1[i-1] > cur[1]) {
