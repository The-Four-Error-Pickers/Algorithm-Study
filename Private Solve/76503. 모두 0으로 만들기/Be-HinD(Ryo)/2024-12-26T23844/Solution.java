    ...
    static void makeTree(Tree currentNode, Tree parent) {
        ...
        for (int idx : list.get(currentNode.v)) {
            if (parent == null || idx != parent.v) {
                Tree childNode = new Tree(idx, (int) value[idx]);
                currentNode.addChild(childNode);
                makeTree(childNode, currentNode);
            }
        }
    }
    static long makeZero(Tree current) {
        if(!current.children.isEmpty()) {
            //리프노드가 아닐경우
            int cnt = 0;
            for(int i=0; i<current.children.size(); i++) {
                cnt += makeZero(current.children.get(i));
            }
            value[current.v] += cnt;
            res += Math.abs(value[current.v]); //여기에서도 올리기전에 횟수를 더함.
            return value[current.v];
        }
        else {
            //리프노드일 경우
            long ret = value[current.v];
            res += Math.abs(ret); //여기에서 횟수를 더함.
            value[current.v] = 0;
            return ret;
        }
    }
