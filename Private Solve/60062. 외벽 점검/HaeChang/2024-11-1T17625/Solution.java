...
class Solution {
    ...
    public int solution(int n, int[] weak, int[] dist) {
        ...
        for(int i = 0;i<weak.length;i++) {
            ...
            dfs(weak[i], end, weak, dist, 0, n, 0, dist.length);
        }
        
        return min == 987654321 ? -1: min;
    }
    static void dfs(int s, int e, int[] w, int[] d, int uCnt, int n, int start, int distLen) {
        if(find) return;
        if(s >= e) {
        ...
        if(start >= distLen) return;
        for(int j = 0;j < w.length;j++) {
            int wVal = s + d[start] > n - 1 ? w[j] + n : w[j];
            if (wVal > s + d[start]) {
                dfs(wVal, e, w, d, uCnt + 1, n, start + 1, distLen); 
                break;
            }
        }
    }
