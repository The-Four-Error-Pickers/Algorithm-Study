...
static int min;
    ...
    public int solution(int n, int[] weak, int[] dist) {
        ...
        for(int i = 0;i<weak.length;i++) {
            ...
            int end = (i + weak.length - 1) % (weak.length);
            find = false;
            dfs(i, end, weak, dist, 0, n, dist.length - 1);
        }
    ...
    static void dfs(int s, int e, int[] w, int[] d, int uCnt, int n, int start) {
        ...
        if(start < 0) return;
        int total = ( w[s] + d[start] ) % n;
        int cnt = 0;
        int st = s+1;
        while(cnt++ < w.length - 1) {
            if (total < w[st % w.length]) {
                dfs(st % w.length, e, w, d, uCnt + 1, n, start - 1);
                break;
            }
            st++;
        }
